...
export default function WhyChooseUsView() {
  ...
  let getWhyChooseUsData = async (page = 1, lim = 5, searchTerm = "", statusFilter = "all") => {
    try {
      setLoading(true);
      ...
      const resp = await axios.get(`${apiBaseurl}/whychooseus/view`, { params });
      ...
      if (finResponse && finResponse.status === "success") {
        ...
        setTotalCount(finResponse.totalCount || 0);
        setStaticPath(finResponse.staticPath || "");
      } else {
        toast.error(finResponse?.message || "Failed to fetch why choose us data.");
        setwhychooseusData([]);
        setTotalPage(0);
      }
    } catch (err) {
      toast.error(err?.response?.data?.message || err.message || "Failed to fetch why choose us data.");
      setwhychooseusData([]);
      setTotalPage(0);
    } finally {
      setLoading(false);
  ...
  let multidelete = async () => {
    if (ids.length >= 1) {
      try {
        const resp = await axios.delete(`${apiBaseurl}/whychooseus/multidelete/`, { data: { ids: ids } });
        const finResponse = resp.data;
        if (finResponse && finResponse.status === "success") {
          toast.success(finResponse.message || "Deleted successfully");
          getWhyChooseUsData(currentPage, limit, search, filterStatus);
          setids([]);
        } else {
  ...
  let statusUpdate = async () => {
    if (ids.length >= 1) {
      try {
        const resp = await axios.post(`${apiBaseurl}/whychooseus/statusupdate/`, { ids: ids });
        const finResponse = resp.data;
        if (finResponse && finResponse.status === "success") {
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-700 mb-2 tracking-tight flex items-center gap-2">
            ...
            <Link to={"/whychooseus/view"} className="hover:text-blue-700 transition-colors cursor-pointer">Why Choose Us</Link>
            <span className="text-gray-400">/</span>
        ...
        <div className="bg-white shadow-xl rounded-2xl p-6 border border-gray-200">
          ...
          {showFilter && (
            <div className="mb-4 flex items-center gap-2">
              ...
              <button
                type="button"
                onClick={() => getWhyChooseUsData(1, limit, search, filterStatus)}
                title="Search"
