...
export default function SliderView() {
  ...
  let getSliderData = async () => {
    axios.get(`${apiBaseurl}slider/view`,
      ...
      .catch(() => {
        ...
        setSliderData([]); // fallback to empty array on error
      });
  }
  ...
  let allCheckId = (e) => {
    if (e.target.checked) {
      let allIds = sliderData.map((v) => v._id);
      setids(allIds);
    }
    else {
      setids([]);
  ...
  let multidelete = () => {
    if (ids.length >= 1) {
      axios.delete(`${apiBaseurl}slider/multidelete/`,
        {
          data: { ids: ids }
        }
      )
  ...
  let statusUpdate = () => {
    if (ids.length >= 1) {
      axios.post(`${apiBaseurl}slider/statusupdate/`, { ids: ids })
        .then((response) => response.data)
        .then((finResponse) => {
          toast.success(finResponse.message);
          getSliderData();
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-700 mb-2 tracking-tight flex items-center gap-2">
            ...
            <Link to={"/slider/view"} className="hover:text-blue-700 transition-colors">Slider</Link>
            <span className="text-gray-400">/</span>
