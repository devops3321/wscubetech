...
export default function CategoryView() {
  ...
  // normalize base url and use category base path
  ...
  let getCategoryData = async (page = 1, lim = 5, searchTerm = "", statusFilter = "all") => {
    try {
      setLoading(true);
      const params = {
        page,
      ...
      const resp = await axios.get(`${CATEGORY_BASE}/view`, { params });
      ...
      if (finResponse && finResponse.status === "success") {
        ...
        toast.error(finResponse?.message || "Failed to fetch category data.");
        setCategoryData([]);
        setTotalPage(1);
      }
    } catch (err) {
      toast.error(err?.response?.data?.message || err.message || "Failed to fetch category data.");
      setCategoryData([]);
    } finally {
      setLoading(false);
    }
  ...
  React.useEffect(() => {
    // fetch initial without filters
    getCategoryData(currentPage, limit, search, filterStatus);
  ...
  let allCheckId = (e) => {
    if (e.target.checked) {
      let allIds = categoryData.map((v) => v._id.toString());
      setids(allIds);
    }
    else {
      setids([]);
  ...
  let multidelete = async () => {
    if (ids.length >= 1) {
      try {
        const resp = await axios.delete(`${CATEGORY_BASE}/multidelete/`, { data: { ids: ids } });
        const finResponse = resp.data;
        if (finResponse && finResponse.status === "success") {
  ...
  let statusUpdate = async () => {
    if (ids.length >= 1) {
      try {
        const resp = await axios.post(`${CATEGORY_BASE}/statusupdate/`, { ids: ids });
        const finResponse = resp.data;
        if (finResponse && finResponse.status === "success") {
        ...
        <div className="bg-white shadow-xl rounded-2xl p-6 border border-gray-200">
          ...
          {showFilter && (
            <div className="mb-4 flex items-center gap-2">
              ...
              <button
                type="button"
                onClick={() => getCategoryData(1, limit, search, filterStatus)}
                title="Search"
