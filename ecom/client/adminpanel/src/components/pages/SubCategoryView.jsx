...
export default function SubCategoryView() {
  ...
  let allCheckId = (e) => {
    if (e.target.checked) {
      let allIds = subcategoryData.map((v) => v._id);
      setids(allIds);
    }
    else {
      setids([]);
  ...
  let multidelete = () => {
    if (ids.length >= 1) {
      axios.delete(`${apiBaseurl}subcategory/multidelete/`,
        {
          data: { ids: ids }
        }
      )
  ...
  let statusUpdate = () => {
    if (ids.length >= 1) {
      axios.post(`${apiBaseurl}subcategory/statusupdate/`, { ids: ids })
        .then((response) => response.data)
        .then((finResponse) => {
          toast.success(finResponse.message);
          getSubCategoryData();
