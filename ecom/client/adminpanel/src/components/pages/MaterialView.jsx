...
export default function MaterialView() {
  ...
  let multidelete = () => {
    if (ids.length >= 1) {
      axios.delete(`${apiBaseurl}material/multidelete/`, { data: { ids: ids } })
        .then((response) => response.data)
        .then((finResponse) => {
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-700 mb-2 tracking-tight flex items-center gap-2">
            ...
            <Link to={"/material/view"} className="hover:text-blue-700 transition-colors">Material</Link>
            <span className="text-gray-400">/</span>
        ...
        <div className="bg-white shadow-xl rounded-2xl p-6 border border-gray-200">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            <h2 className="text-3xl font-semibold text-gray-800">View Material</h2>
            ...
            <table className="min-w-full table-auto text-sm">
              <thead>
                <tr className="bg-gradient-to-r from-blue-700 to-blue-500 text-white">
                  ...
                  <th className="px-4 py-3 text-left w-3/12">MATERIAL NAME</th>
                  ...
                  materialData.length >= 1 ?
                    materialData.map((row, idx) => (
                      <tr
                        ...
                        <td className="px-4 py-2 align-middle font-medium">{row.categoryName}</td>
                        ...
                        <td className="px-4 py-2 align-middle text-center">
                          <div className="flex items-center justify-center h-full">
                            <Link to={`/editmaterial/${row._id}`}>
                              <button className="bg-blue-600 hover:bg-blue-700 text-white p-2 rounded-full shadow transition-colors duration-200 cursor-pointer" title="Edit Material">
                                <FaPen />
                              </button>
                            </Link>
                          </div>
